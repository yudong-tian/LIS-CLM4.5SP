...
!YDT
   if(masterproc) write(iulog,*) 'clm_drv():  after dynland_hwcontent() loop' 
...
!YDT
   if(masterproc) write(iulog,*) 'clm_drv():  after decomp_vertprofiles() loop' 
...
!$OMP PARALLEL DO PRIVATE (nc,g,begg,endg,begl,endl,begc,endc,begp,endp)
  do nc = 1,nclumps
     ...
     call t_stopf('begcnbal')
      ...
      !YDT
      if(masterproc) write(iulog,*) 'clm_drv():  after BeginWaterBalance() loop' 
!$OMP END PARALLEL DO
...
!$OMP PARALLEL DO PRIVATE (nc,l,c,begg,endg,begl,endl,begc,endc,begp,endp)
  do nc = 1,nclumps
   ...
   ! CH4
     ...
     !YDT
     if(masterproc) write(iulog,*) 'clm_drv():  after all the physics loop' 
