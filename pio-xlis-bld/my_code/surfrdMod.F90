...
!YDT
    ...
    if (masterproc) then
       write(iulog,*)'lat/lon grid flag (isgrid2d) is ',isgrid2d, " ni=", ni, " nj=", nj, " ns=", ns
    end if
...
!-----------------------------------------------------------------------
    ...
    call get_proc_bounds(beg, end)
            ! YDT
            if (masterproc) then
               write(iulog,*)"surfrd_get_grid: beg=", beg, " end=", end 
            end if

    call domain_init(ldomain, isgrid2d=isgrid2d, ni=ni, nj=nj, nbeg=beg, nend=end)
    ! after domain_init, ldomain was allocated with its members as 1D array(beg:end), such as ldomain%mask(beg:end) 
    ...
    if (istype_domain) then
       ...
       if (.not. readvar) call endrun( trim(subname)//' ERROR: yc NOT on file' )
         !YDT tmp check 
         if (masterproc) then
            write(iulog,*)trim(subname), " precheck0 ldomain" 
       ...
       if (.not. readvar) call endrun( trim(subname)//' ERROR: LATIXY NOT on file' )
         ...
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " between precheck0 and precheck1 ldomain" 

    if (isgrid2d) then
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d  <0>, ni=" , ni, " nj=", nj

       !YDT allocate(rdata2d(ni,nj), lon1d(ni), lat1d(nj))
       allocate(lon1d(ni), stat=ier)
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d, lon1d allocated, ier= ", ier
       allocate(lat1d(nj), stat=ier)
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d, lat1d allocated, ier= ", ier
       
       allocate(rdata2d(ni,nj), stat=ier)
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d, rdata2d allocated, ier=", ier
       ...
       if (istype_domain) then
         ...
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d before <1>" 
       ...
       lon1d(:) = rdata2d(:,1)
         ...
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d <1>" 
       ...
       deallocate(rdata2d)
         ...
         !YDT tmp check 
         if (masterproc) &  
            write(iulog,*)trim(subname), " inside isgrid2d <2>" 

    end if

    !YDT tmp check 
    if (masterproc) then
       write(iulog,*)trim(subname), " precheck1 ldomain" 
       call domain_check(ldomain)
    endif
    ...
    !YDT tmp check 
    if (masterproc) then
       write(iulog,*)trim(subname), " precheck2 ldomain" 
       call domain_check(ldomain)
    endif
...
!-----------------------------------------------------------------------
    ...
    call get_proc_bounds(begg,endg)
            ! YDT
            if (masterproc) then
               write(iulog,*)"surfrd_get_data: begg=", begg, " endg=", endg 
            end if

    allocate(pctspec(begg:endg))    !  YDT percent of spec lunits wrt gcell
    ...
    !YDT
    if (masterproc) write(iulog, *) "Reading PFTDATA_MASK: ", trim(locfn)
...
! !ARGUMENTS:
    ...
    type(file_desc_t),intent(inout) :: ncid   ! netcdf id
    integer          ,intent(in)    :: ns     ! domain size
    integer          ,pointer       :: pftm(:)   !YDT: pft mask 
!
! !CALLED FROM:
! subroutine surfrd in this module
!
...
!-----------------------------------------------------------------------
    ...
    !YDT
    call get_proc_bounds(begg,endg)
        write(iulog,*) "begg=", begg, " endg=", endg, " numpft=", numpft 
    ...
    do nl = begg,endg
       if (pftm(nl) >= 0) then
          ...
          !YDT real(r8), allocatable :: pctspec()   ! percent of spec lunits wrt gcell
          if (pctspec(nl) < 100._r8 * (1._r8 - eps_fact*epsilon(1._r8))) then  ! pctspec not within eps_fact*epsilon of 100
             sumpct = 0._r8
             do m = 0,numpft
               !YDT 
#ifdef YDT_DEBUG
                write(iulog,*) "pctpft(nl, m)=", pctpft(nl, m), " pctspec(nl)=", pctspec(nl)
#endif
             ...
             if (abs(sumpct - 100._r8) > 0.1e-4_r8) then
                ...
                write(iulog,*) sumpct, sumpct-100._r8, nl, iam
                call endrun()
             end if
