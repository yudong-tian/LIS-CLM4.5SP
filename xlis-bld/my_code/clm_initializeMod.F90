...
!-----------------------------------------------------------------------
    ...
    if (masterproc) then
       write(iulog,*) 'Skipping ncd_pio_init  .....'
       write(iulog,*)
       call shr_sys_flush(iulog)
    endif
    !YDT 12/2/15
    ...
    if (iam == 1) then 
      write(iulog, *) " =========== Proc info for cpu 1 ==============" 
      write(iulog, *) "nclumps  ncells   nlunits    ncols    npfts   begg    endg    begl    endl   "
      write(iulog, '(9I8)') procinfo%nclumps, procinfo%ncells, procinfo%nlunits,  procinfo%ncols, &
                      procinfo%npfts,  procinfo%begg, procinfo%endg, procinfo%begl, procinfo%endl
      write(iulog, *) "cids=", procinfo%cid 
      !write(iulog, *) "gdc2glo: size=", size(ldecomp%gdc2glo), " values=",  ldecomp%gdc2glo  
      !write(iulog, *) "glo2gdc: size=", size(ldecomp%glo2gdc), " values=",  ldecomp%glo2gdc  
   end if
    ...
    if (create_glacier_mec_landunit) then
       ...
       ! YDT
       if (masterproc) then
            write(iulog,*) 'about to call surrd_get_grid ',trim(fatmlndfrc)
            call shr_sys_flush(iulog)
       endif
    ...
    !YDT 
       if (masterproc) then
          write(iulog,*) 'After initGridCells: ====================' 
          write(iulog,*) 'pft%wtgcell() range: ', lbound(pft%wtgcell, 1), ubound(pft%wtgcell, 1), minval(pft%wtgcell), maxval(pft%wtgcell) 
          write(iulog,*) 'pft%wtlunit() range: ', lbound(pft%wtlunit, 1), ubound(pft%wtlunit, 1), minval(pft%wtlunit), maxval(pft%wtlunit) 
          write(iulog,*) 'pft%wtcol() range:', lbound(pft%wtcol, 1), ubound(pft%wtcol, 1), minval(pft%wtcol), maxval(pft%wtcol)
          call shr_sys_flush(iulog)
       endif
...
!YDT
       if (masterproc) write(iulog,*)'Inside initialize2: before timemgr_init() '
...
!YDT
    if (masterproc) write(iulog,*)'Inside initialize2: before hist_htapes_build() '

    if (nsrest == nsrStartup .or. nsrest == nsrBranch) call hist_htapes_build()

!YDT
    if (masterproc) write(iulog,*)'Inside initialize2: after hist_htapes_build() '
